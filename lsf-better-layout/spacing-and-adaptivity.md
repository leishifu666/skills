...
WCAG target-size requirements, larger usability targets and pseudo-element expansion belong to `lsf-better-accessibility`; these clearances are in addition, so expanded hit areas never overlap.
...
- **Truncation cues.** Clamped text shows an ellipsis and a way to expand. Truncation mechanics are `lsf-better-typography`'s.
