...
- Numbers in tables align to the trailing edge, text to the leading edge. Tabular figures are `lsf-better-typography`'s.
...
Where arrangement encodes progression, as in star ratings, step indicators and progress bars, the sequence mirrors in RTL and stars fill from the trailing side. Flexbox and grid with logical properties mirror automatically; hand-positioned elements do not. Digit order inside numbers never reverses, which with other bidi rules belongs to `lsf-better-typography`.
...
- One primary action per view. `lsf-better-colors` owns how color enforces it.
