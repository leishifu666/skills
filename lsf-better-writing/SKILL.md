---
name: lsf-better-writing
title: 技能：LSF Better Writing
description: UX 写作与界面文案。用于撰写或审查任何面向用户的文本。 触发词：UX 写作、微文案、按钮标签、链接文本、错误信息、空状态、占位文本、设置标签、大小写、sentence
  case、品牌声音与语气。
---
...
How copy renders (capitalization via `text-transform`, truncation, smart punctuation) belongs to `lsf-better-typography`. Error markup and announcements (`aria-invalid`, live regions) belong to `lsf-better-accessibility`. Room for translated strings belongs to `lsf-better-layout`.
