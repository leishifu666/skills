...
name: lsf-break
title: 技能：LSF Break
description: 回答一个组件“扛得住真实使用吗”。把它在所有真实可达状态中渲染到页面，并把页面作为可视化报告交给用户查看哪里损坏。
...
It observes rather than judges. A finding here is something that visibly broke on the page, named in the vocabulary of the domain skill that owns the fix. Reviewing code against a standard is `lsf-interface-review` and `lsf-better-interface`; exploring design alternatives is `lsf-variant`.

Where `lsf-variant` insists on the real page, this skill isolates on purpose. You are not judging how the component looks in context. You are checking whether it defends itself when the content is worst-case.
...
| --- | --- | --- |
| One unbreakable 60-character string | Overflows the card, no wrap and no truncation | `lsf-better-typography` |
| Zero items | Blank region with no message | `lsf-better-writing` |
