...
def safe_target(root: Path, member: str) -> Path:
    ...
    win = PureWindowsPath(member)
    if win.drive or win.root or ".." in win.parts or ":" in member:
...
def main() -> None:
    ...
    destination = backend_root / (lock["tag"] + "-" + key)
    marker = destination / ".qiaomu-install.json"
    if marker.is_file():
        installed = json.loads(marker.read_text(encoding="utf-8"))
        files = installed.get("files") or {}
        valid_files = bool(files) and all(safe_target(destination, name).is_file() and sha256(safe_target(destination, name)) == digest for name, digest in files.items())
        if valid_files and installed.get("archive_sha256") == asset["sha256"] and installed.get("platform") == key:
    ...
    try:
        ...
        file_hashes = {str(path.relative_to(stage)): sha256(path) for path in stage.rglob('*') if path.is_file()}
        marker_payload = {"files": file_hashes, "tag": lock["tag"], "platform": key, "asset": asset["name"], "archive_sha256": actual, "source": url}
