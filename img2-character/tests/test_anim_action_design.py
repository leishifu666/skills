...
Falls back to a local §1/§2 implementation when `anim_clip_features.py` is not importable, so this
...
Ported from the base skill's forge/tests/test_action_design.py. Its subject moved to this plugin as
`tools/anim_action_design.py` (PLUGIN_CONTRACT.md §4, §8), so the assertions that pin that module's
...
The CLI tests still shell out to the module file, now `tools/anim_action_design.py`; that module
carries no img2 bootstrap stanza, so the in-process imports here work unchanged.
"""
...
MODULE = Path(__file__).resolve().parents[1] / "tools" / "anim_action_design.py"


# ---------------------------------------------------------------------------------------------
# §1 / §2 fallback — used only when anim_clip_features.py is absent.
#
# This is a deliberate, minimal transcription of the spec's own tables, kept in the TEST so the
# suite is self-contained while `anim_clip_features.py` (owned by another author) lands. It is NOT a
# second implementation for production use: anim_action_design imports the real one and this file
# prefers it whenever it is importable.
# ---------------------------------------------------------------------------------------------


class _FallbackFeatures:
    """§1's feature vector, normalised by H exactly as `anim_clip_features.measure_clip` does."""
...
class FallbackAndHelpers(unittest.TestCase):
    ...
    def test_authored_clips_validate_against_the_contract_payload_shape(self) -> None:
        ...
        if USING_REAL_CLIP_FEATURES:
            from anim_clip_features import load_payload  # noqa: PLC0415
...
class Cli(unittest.TestCase):
    ...
    def test_cli_accepts_a_good_walk_and_exits_zero(self) -> None:
        if not USING_REAL_CLIP_FEATURES:
            self.skipTest("the payload path of the CLI needs anim_clip_features.measure_clip")
        _, clip = authored(walk_targets(H))
...
if __name__ == "__main__":
    print(
        f"anim_clip_features {'IS' if USING_REAL_CLIP_FEATURES else 'is NOT'} importable; "
