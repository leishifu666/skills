...
(tools/rig_emit_rig.py), executes that emitted code with the
...
Ported from the base skill's forge/tests/test_rig_milestone0.py. Its subject moved to this plugin as
`tools/rig_emit_rig.py` (PLUGIN_CONTRACT.md §4, §8), so the assertions that pin that module's
...
`showcase_root()` is inlined verbatim from the base suite's forge/tests/showcase_test_support.py
rather than ported as a second file: this is the only test that needs it, and a plugin's tests/
directory should not grow a support module for one caller.
"""
...
from rig_emit_rig import emit_typescript  # noqa: E402
from rig_spec import BoneSpec, RigSpec  # noqa: E402


def showcase_root() -> Path:
    """Verbatim from the base suite's forge/tests/showcase_test_support.py."""
