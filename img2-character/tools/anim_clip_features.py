...
Ported from the base skill's forge/stage5_rig/clip_features.py to this plugin's tools/anim_clip_features.py
...
# a file called `clip-features.json` may hold anything, so the payload says what it is and a
# reader that wants something else refuses instead of misreading it.
# ------------------------------------------------------------------------------------------

PLUGIN_ID = "character"
ARTIFACT_KIND = "rigging.clip-features-v1"
ARTIFACT_NAME = "clip-features.json"
...
# §4 loop rule. These two are named in CONTRACT_1.5.2.md and shared with rig_gate_core.
...
def main(argv: list[str], workspace_required: bool = False) -> int:
    """Measure a payload and report it; write the artifact where the workspace says it belongs.
    ...
    parser = argparse.ArgumentParser(
        description="Measure, classify, name and loop-decide every clip in a sampled-clip payload."
    )
    parser.add_argument("payload", help="CONTRACT_1.5.2 sampled-clip payload (JSON)")
    ...
    if args.out is not None:
        ...
        try:
            ...
            print(f"anim-clip-features: {err}", file=sys.stderr)
            return 2
    ...
    try:
        ...
        # Nothing is written on a bad payload. A stale artifact left in place is a lie the next
        # reader has no way to spot; an absent one is a question they can answer.
