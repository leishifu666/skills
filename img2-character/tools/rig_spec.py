...
Ported from the base skill's forge/stage5_rig/rig_spec.py to this plugin's tools/rig_spec.py
...
def derive_envelope_radius(
    ...
    DELIBERATE DUPLICATE. This is a byte-for-byte copy of the same function in
    the base skill's forge/stage5_rig/rig_spec.py, and both copies are live: the
    base imports its own at forge/stage3_build/generate_threejs_factory.py:2106
    (a deferred import inside a function body), this one is imported by this
    plugin's tools/rig_emit_rig.py. The copy is not an oversight -- a plugin may
    not import forge.* (PLUGIN_CONTRACT.md section 8), so the §4.3 formula has
    to exist here in full or not at all.

    Two live copies of one formula drift. tests/test_rig_spec_agreement.py is
    what stops that: it loads the base file by absolute path (no forge import),
    sweeps both implementations over the same inputs, and fails when they
    disagree. Change this function only together with the base's, and let that
    test confirm they still agree.
    """
