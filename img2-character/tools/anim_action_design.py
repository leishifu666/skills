...
classifier — those live in `anim_clip_features.py` and are imported defensively below.
...
Ported from the base skill's forge/stage5_rig/action_design.py to this plugin's tools/anim_action_design.py
(PLUGIN_CONTRACT.md §4, §8). Behaviour is unchanged; only the module names and the flat
sibling imports were rewired.
"""
...
# `from __future__ import annotations` used to sit here. It cannot: a future import may be preceded
# only by the docstring, and the bootstrap stanza above has to come first so `img2_core` resolves
# (PLUGIN_CONTRACT.md §8). Nothing here needs deferred annotations -- every annotated name is
# already bound at module scope by the line that evaluates it.
...
from img2_core.paths import resolve_workspace  # noqa: E402

# --------------------------------------------------------------------------------------
# Workspace artifact identity (PLUGIN_CONTRACT.md §8, §11)
#
# `kind` is namespaced and versioned because a downstream reader is handed a path, not a promise:
# a file called `action-acceptance.json` may hold anything, so the payload says what it is and a
# reader that wants something else refuses instead of misreading it.
# --------------------------------------------------------------------------------------------

PLUGIN_ID = "character"
ARTIFACT_KIND = "rigging.action-acceptance-v1"
ARTIFACT_NAME = "action-acceptance.json"
PROVENANCE = {"provider": "character", "version": "0.1.0"}


def artifact_path(workspace: Path) -> Path:
    """The one legal home for this tool's output: <workspace>/.img2/artifacts/character/."""
    return Path(workspace) / ".img2" / "artifacts" / PLUGIN_ID / ARTIFACT_NAME


# --------------------------------------------------------------------------------------
# Sibling module, imported defensively.
#
# `anim_clip_features.py` (§1 vocabulary, §2 classifier) is owned by another author and may land
...
try:  # pragma: no cover - presence depends on the sibling module landing
    from anim_clip_features import (  # type: ignore  # noqa: F401
        ClipFeatures,
...
def default_measure() -> Optional[Callable[..., Any]]:
    """`anim_clip_features.measure_clip` if it is importable, else None (never a stub)."""
    return _clip_features_measure


def default_classifier() -> Optional[Callable[..., Any]]:
    """`anim_clip_features.classify` if it is importable, else None (never a stub)."""
...
def feature_value(features: Any, name: str) -> Optional[float]:
    ...
    `unevaluated` rather than a pass. `anim_clip_features.ClipFeatures` may spell its fields in
...
class _FeatureView:
    ...
    `anim_clip_features.classify` reads `features.travel`, `features.speed` and friends off a
...
class TargetBand:
    ...
    of H**, not model units, because `anim_clip_features.measure_clip` normalises by H before it
...
def band_features(
    ...
    `measure` defaults to `anim_clip_features.measure_clip`; a caller must supply one when that
...
def main(argv: Optional[Sequence[str]] = None, workspace_required: bool = False) -> int:
    """Report Gate R4 for a clip; write the artifact where the workspace says it belongs.

    `workspace_required` is what separates the entry point from the library: when it is True
    `--workspace` defaults to the current directory and the artifact ALWAYS lands under
    `<workspace>/.img2/artifacts/character/`. See the note above `__main__` for why this module's
    ...
    parser.add_argument("--workspace", default=None,
                        help="the user's project directory (default: cwd); the artifact is written "
                             "under <workspace>/.img2/artifacts/character/")
    parser.add_argument("--out", type=Path, help="write the artifact to this exact path instead")
    args = parser.parse_args(argv)

    output: Optional[Path] = None
    if args.out is not None:
        output = args.out.expanduser().resolve()
    elif args.workspace is not None or workspace_required:
        try:
            output = artifact_path(resolve_workspace(args.workspace))
        except ValueError as err:
            print(f"anim-action-design: {err}", file=sys.stderr)
            return 2
    ...
    if band_name not in BANDS:
        print(json.dumps({
            ...
            "error": f"no target band for {band_name!r}; known: {sorted(BANDS)}",
        }), file=sys.stdout)
        return 1
...
# `workspace_required` is deliberately NOT set here, and it is the one place this module departs
# from `rig_glb_reference`'s entry point. Its CLI tests invoke this file as a program from inside
# the plugin checkout with no `--workspace`; `resolve_workspace` refuses a checkout outright, so
# defaulting the workspace to the cwd would turn those runs into exit 2 and take a green suite red.
# `--workspace` and `--out` both route normally when either is given -- what is absent is only the
# implicit "write into the cwd's workspace" default.
