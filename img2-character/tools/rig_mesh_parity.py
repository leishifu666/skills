...
Ported from the base skill's forge/stage5_rig/mesh_parity.py to this plugin's tools/rig_mesh_parity.py
(PLUGIN_CONTRACT.md §4, §8). Behaviour is unchanged; only the module name changed.
"""
...
#
# `verify` is handed a PATH to a manifest, not a promise about what is in it. A file called
# `mesh-manifest.json` may hold anything -- last week's manifest, a summary, some other tool's
# report -- so the manifest states its own `kind` and `Manifest.from_dict` refuses anything else
# instead of reading a stranger's JSON as a freeze and reporting a pass over it.
# ---------------------------------------------------------------------------------------------

PLUGIN_ID = "character"
ARTIFACT_KIND = "rigging.mesh-manifest-v1"
ARTIFACT_NAME = "mesh-manifest.json"
# The summary is a DIFFERENT document -- the same freeze with its buffers stripped -- so it carries
# its own kind. Feeding it to `verify` has to fail loudly; it cannot prove anything.
SUMMARY_KIND = "rigging.mesh-manifest-summary-v1"
...
class Manifest:
    ...
    def from_dict(payload: dict[str, Any]) -> "Manifest":
        ...
        # The kind is checked BEFORE anything is read out of the payload. A summary document has a
        # `meshes` array too, and every entry in it has a `name` and a `hashes` map -- so a summary
        # parsed as a manifest would produce a Manifest whose retained buffers are all empty, and
        # `verify` would then report a clean pass over a document that proves nothing. Refusing on
        # the kind is the only check that catches that before it becomes a false green.
        kind = payload.get("kind")
        if kind != ARTIFACT_KIND:
            raise ValueError(
                f"this document is {kind!r}, not {ARTIFACT_KIND!r}; verify reads the freeze "
                f"manifest written by `freeze`, and a file name is not a contract"
            )
...
def _freeze_output(args: argparse.Namespace) -> Path | None:
    """Where the manifest goes: an explicit `--out`, else the workspace, else nowhere.

    `--out` still wins outright -- a caller that named a path gets that path. Otherwise the
    artifact lands under `<workspace>/.img2/artifacts/character/`, never in whatever directory the
    tool happened to be started from. `workspace_required` (set only by `__main__`) is what makes
    the run-as-a-program case always write: called in-process with neither flag, the library
    caller gets stdout and no file.
    """
    if args.out is not None:
        return args.out.expanduser().resolve()
    if args.workspace is not None or args.workspace_required:
        return artifact_path(resolve_workspace(args.workspace))
    return None


def _run_freeze(args: argparse.Namespace) -> int:
    output = _freeze_output(args)
...
def main(argv: list[str], workspace_required: bool = False) -> int:
    ...
    freezer.add_argument("--workspace", default=None,
                         help="the user's project directory (default: cwd); the manifest is written "
                              "under <workspace>/.img2/artifacts/character/")
    freezer.add_argument("--out", type=Path, help="write the full manifest to this exact path instead")
...
if __name__ == "__main__":
    raise SystemExit(main(sys.argv[1:], workspace_required=True))
