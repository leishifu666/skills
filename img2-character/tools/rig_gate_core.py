...
DELEGATION. This module owns no measurement it can borrow:
  G4/G5 -> `rig_skin_conditioning.validate_binding` (Gate R2 lives there)
  G7    -> `anim_action_design.check_medial_lateral`
  G8    -> `anim_action_design.foot_slide` / `anim_action_design.FOOT_SLIDE_LIMIT`
  G9    -> `anim_clip_features.measure_clip(...).scale_delta`
...
8. G7 and the tautological pass. `anim_action_design.Chains.gate_is_independent` records something the
...
10. G11 and a manifest/payload disagreement about which meshes exist. `rig_mesh_parity.verify`
    ...
    `rig_mesh_parity.verify` and adds no parity rule of its own; re-deriving one here is how a gate and
...
11. G11 and legally-added attributes. `skinIndex` and `skinWeight` are deliberately outside
    ...
    `rig_mesh_parity` reports are carried into `measured.addedLegal` for the reader and NEVER
...
12. G12 and the GLB report's `ok` field. `GlbRig.to_dict()` does not emit `ok`; the field is added
    by `rig_glb_reference`'s CLI (`result["ok"] = not errors`), so an in-process caller's report
...
13. G12 and unsupported interpolation. The R6 brief groups "no skin, multiple skins without a
    choice, unsupported interpolation" under `structuralFailure`, but `rig_glb_reference` sets
...
Ported from the base skill's forge/stage5_rig/rig_gates.py to this plugin's tools/rig_gate_core.py
...
from anim_action_design import (  # noqa: E402
    FOOT_SLIDE_LIMIT,
...
from anim_clip_features import measure_clip  # noqa: E402
...
# rig_mesh_parity also exports a `load_payload`; importing them flat would shadow ours with
# constants that happen to agree today and might not tomorrow.
import rig_glb_reference  # noqa: E402
import rig_mesh_parity  # noqa: E402
from rig_skin_conditioning import SkinBinding, WEIGHT_SUM_TOLERANCE, validate_binding  # noqa: E402

# ---------------------------------------------------------------------------------------------
# Tolerances. The names are the spec's and the values come from CONTRACT_1.5.2.md; the two that
# rig_skin_conditioning and anim_action_design own are IMPORTED, never restated, so a change there cannot
...
# ---------------------------------------------------------------------------------------------
# G4 / G5 — delegated to rig_skin_conditioning.validate_binding (Gate R2)
# ---------------------------------------------------------------------------------------------
...
def gate_g4(payload: Mapping[str, Any], verdict: _BindingVerdict | None = None) -> GateResult:
    """|1 - sum(w)| <= 2e-7 every vertex. Delegated in full to `rig_skin_conditioning.validate_binding`.
...
def gate_g5(payload: Mapping[str, Any], verdict: _BindingVerdict | None = None) -> GateResult:
    """maxSkinIndex <= bones - 1. Delegated in full to `rig_skin_conditioning.validate_binding`.
...
# ---------------------------------------------------------------------------------------------
# G9 — no joint scale (delegated measurement: anim_clip_features.measure_clip)
# ---------------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------
# G7 — medial/lateral (delegated to anim_action_design.check_medial_lateral)
# ---------------------------------------------------------------------------------------------


def gate_g7(payload: Mapping[str, Any]) -> GateResult:
    """leftAnchor.x > 0 > rightAnchor.x. Delegated to `anim_action_design.check_medial_lateral`.
    ...
    AMBIGUITY 8 -- THE TAUTOLOGY. `anim_action_design.Chains.gate_is_independent` records something the
...
# ---------------------------------------------------------------------------------------------
# G8 — foot contact (delegated to anim_action_design.foot_slide)
# ---------------------------------------------------------------------------------------------


def gate_g8(payload: Mapping[str, Any]) -> GateResult:
    """footSlide <= 0.01H in stance. Delegated per clip to `anim_action_design.foot_slide`.
...
# ---------------------------------------------------------------------------------------------
# G11 — mesh parity (delegated to rig_mesh_parity.verify)
# ---------------------------------------------------------------------------------------------


def gate_g11(payload: Mapping[str, Any]) -> GateResult:
    """Every frozen buffer byte-identical after binding. Delegated to `rig_mesh_parity.verify`.
    ...
    THIS GATE ADDS NO PARITY RULE OF ITS OWN (ambiguity 10). `rig_mesh_parity` decides what counts as a
    ...
    threshold = {
        "frozenBuffers": list(rig_mesh_parity.FROZEN_BUFFERS),
    ...
    try:
        manifest = rig_mesh_parity.Manifest.from_dict(dict(raw_manifest))
    except Exception as exc:
        return _unevaluated(
            spec,
    ...
    try:
        report = rig_mesh_parity.verify(manifest, dict(after))
    except Exception as exc:
        return _unevaluated(
            spec,
    ...
    if report.status == rig_mesh_parity.STATUS_UNEVALUATED:
        return _unevaluated(
            spec,
    ...
    if report.status == rig_mesh_parity.STATUS_FAIL:
...
def gate_g12(payload: Mapping[str, Any]) -> GateResult:
    ...
    threshold = {
        ...
        "supportedInterpolations": list(rig_glb_reference.SUPPORTED_INTERPOLATIONS),
    }
    block = payload.get(spec.input_key)
    if not isinstance(block, Mapping):
        return _unevaluated(
    ...
    if unsupported:
        return _verdict(
            ...
            f"{list(rig_glb_reference.SUPPORTED_INTERPOLATIONS)} "
