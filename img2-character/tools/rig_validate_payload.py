...
Ported from the base skill's forge/stage5_rig/validate_rig_payload.py to this plugin's tools/rig_validate_payload.py
...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(parent_of, list) or len(parent_of) != joint_count:
        errors.append("parents must have one entry per joint")
        parent_of = parent_of if isinstance(parent_of, list) else []
    ...
    allow_multiple_roots = payload.get("allowMultipleRoots") is True
    admitted_zero_length = payload.get("allowZeroLengthBones", [])
    if not isinstance(admitted_zero_length, list) or not all(
        isinstance(index, int) and not isinstance(index, bool) and 0 <= index < joint_count
        for index in admitted_zero_length
    ):
        errors.append("allowZeroLengthBones must be an array of valid joint indices")
        admitted_zero_length = []
    root_indices = [index for index, parent in enumerate(parent_of) if parent is None]
    if parent_of and parent_of[0] is not None:
        errors.append("parent_of[0] must be null for the first root")
    if len(root_indices) > 1 and not allow_multiple_roots:
        errors.append(
            "multiple root joints require allowMultipleRoots=true; "
            f"found roots {root_indices}"
        )
    for index, parent in enumerate(parent_of[1:], start=1):
        if parent is None and allow_multiple_roots:
            continue
        if not isinstance(parent, int) or isinstance(parent, bool):
            errors.append(f"parent_of[{index}] must be an integer parent index or an admitted null root")
        elif parent < 0 or parent >= index:
            errors.append(f"parent_of[{index}] must satisfy 0 <= parent < child index")
        elif (
            index < len(joints)
            and parent < len(joints)
            and distance(joints[index], joints[parent]) <= EPSILON
            and index not in admitted_zero_length
        ):
    ...
    return {
        ...
        "summary": {
            ...
            "rootJointCount": len([parent for parent in parent_of if parent is None]),
            "admittedZeroLengthBoneCount": len(admitted_zero_length),
