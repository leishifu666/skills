---
name: lsf-variant
title: 技能：LSF Variant
description: 回答“这几个选哪个”，而不是“这个对不对”。在真实页面中为一块 UI 构建多个真正不同的版本，通过选择器切换比较并把胜出方案转为正式实现。
...
Every other skill here judges. This one produces candidates and hands the decision back. Reviewing existing UI is `lsf-interface-review` and `lsf-better-interface`, and it does not pick the winner.
...
| --- | --- | --- |
| Structure | `lsf-better-layout` | Grouping, order, column count, what collapses |
| Density | `lsf-better-layout` | Spacing scale, hit areas, how much fits |
| Emphasis | `lsf-better-colors` | Where filled color goes, what recedes |
| Type | `lsf-better-typography` | Scale steps, weight contrast, measure |
| Voice | `lsf-better-writing` | Labels, tone, how much copy |
...
Before a variant enters the picker it clears `lsf-better-interface`'s escalation triggers. Every control has an accessible name, keyboard reaches everything a pointer does, focus is visible, nothing clips at 320px and no meaning rides on color alone.
