---
name: lsf-space-video-transcript
description: LSF｜从 YouTube、小宇宙、B站、抖音和小红书链接提取字幕或逐字稿；优先读取平台字幕，无字幕时自动提取音频并调用可用 ASR，输出
  Markdown 与 SRT。用户发送这些平台链接、要求提取字幕/文案/逐字稿/视频转文字，或在字幕完成后希望下载原视频时使用。字幕成功后必须询问用户是否下载原视频；小宇宙对应下载原音频。
title: LSF 视频转写
metadata:
  github_url: https://github.com/SpaceZephyr/creator-buddy
  github_hash: edf46c567ff54b72ee2157d06f3a02dbd67aaa9c
  github_path: video-Skills/space-video-transcript
  upstream_name: space-video-transcript
...
> **LSF 汉化版**：上游模块名 `space-video-transcript`，本机调用名 `lsf-space-video-transcript`。
