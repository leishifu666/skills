...
**Document Illustrator** 是一个 Antigravity Skill，它能智能理解文档内容，自动归纳核心要点，并为每个主题生成专业配图。无需依赖特定文档格式，AI 会理解内容并生成符合你选择风格的高质量图片。
...
```bash
cd ~/.gemini/antigravity/skills/document-illustrator
...
### 方式 1: 通过 Antigravity 使用（推荐）

这是主要的使用方式，让 Antigravity 智能处理整个流程。
...
3. **指定配图数量**
   - 根据文档长度，Antigravity 会推荐合适的数量
   - 你可以自定义需要的图片数量（建议 3-10 张）

4. **确认内容归纳**
   - Antigravity 会展示归纳的主题列表
   - 确认所有重要内容都被包含
   - 可以要求调整归纳结果
...
├── LICENSE                   # MIT 许可证
├── SKILL.md                  # Skill 定义文件（供 Antigravity 使用）
├── .env                      # API 密钥配置（需自行创建）
├── .gitignore                # Git 忽略规则
├── scripts/                  # Python 脚本目录
...
**A**: 可以尝试：
1. 在归纳展示阶段，告诉 Antigravity 你的期望，它会重新归纳
2. 尝试不同的风格
3. 调整配图数量（增加或减少）
4. 提供更详细的文档内容
...
**A**: 目前推荐通过 Antigravity 逐个处理文档。如果需要批量处理，可以编写自定义脚本循环调用 `generate_single_image.py`。
...
| **支持文档格式** | Markdown, 纯文本, PDF 等（任何 Antigravity 可读的格式） |
| **平均生成时间** | 10-20 秒/张 |
| **Python 版本** | 3.8+ |
...
- **Claude Sonnet 4.5 / Gemini Pro** - AI 智能内容分析和归纳
- **Gemini 3 Pro Image Preview** - 高质量图片生成
- **Antigravity** - Skill 执行环境
