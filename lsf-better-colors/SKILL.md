---
name: lsf-better-colors
title: 技能：LSF Better Colors
description: 数字产品的颜色系统。用于创建或扩展色板、设计明暗主题，或审查代码中的颜色。触发词：色板、色板生成、色阶、品牌色、强调色、灰阶、状态色、颜色
  token、token 命名、主题化、深色模式、对比度、APCA、色域、Display P3、OKLCH、颜色转换、渐变、颜色语义、增强对比度。
---
...
Contrast requirements belong to `lsf-better-accessibility`. Surfaces, shadows and icon color belong to `lsf-better-ui`.
...
Use a color for one purpose across the whole interface, treating anything within `15°` of hue as the same color. If the accent means interactive, that hue on static text tells users to click something that is not clickable, and an interactive element rendered neutral misleads just as badly. Color is never the only carrier of meaning, which `lsf-better-accessibility` owns.
