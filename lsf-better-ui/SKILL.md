---
name: lsf-better-ui
title: 技能：LSF Better UI
description: 让界面更精致的设计工程原则。用于构建 UI 组件、实现动画或 hover 状态，以及处理视觉细节。触发词：UI 质感、感觉不对、交错动画、入场动画、主题切换过渡、圆角、光学对齐、图片描边、阴影、图标、图标线宽、动效克制。
---
...
Text wrapping, font rendering, tabular numbers and text spacing belong to `lsf-better-typography`. Hit areas, focus, keyboard support, ARIA and reduced motion belong to `lsf-better-accessibility`. Grouping, section spacing, breakpoints and spatial RTL belong to `lsf-better-layout`.
