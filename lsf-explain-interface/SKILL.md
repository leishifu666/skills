---
name: lsf-explain-interface
title: LSF Explain 界面
description: 回答界面“这是怎么做出来的”。提供 URL 并指出感兴趣的部分后，读取产生该效果的各层实现；未指定部分时读取整个前端；面对截图时明确说明是重建而非读取源码。
...
This skill answers how something was built. `/lsf-explain-interface how the gradient on example.com was built` finds the layers producing that gradient and explains what each one contributes.

It explains rather than judges. There is no verdict, because how someone else built their interface is not a finding. Reviewing against a standard is `lsf-interface-review` and `lsf-better-interface`; exploring alternatives for your own is `lsf-variant`.
