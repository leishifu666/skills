...
Sample the actual pixels rather than describing what you see. Then hand the values to `lsf-better-colors`:
...
- **Measure:** count characters on a full line of body copy and compare against `lsf-better-typography`'s 60 to 75 range.
...
Then check the ratio `lsf-better-layout` owns, the gap between groups against the gap within one. At or above 2, space is carrying the grouping. Below that, something else is, usually a border or a background shape.
