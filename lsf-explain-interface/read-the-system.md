...
Group the result by prefix. The prefixes are the system's own layer names, and a two-tier structure, `--blue-500` feeding `--color-text-primary`, is the seam `lsf-better-colors` calls the semantic tier.
...
Look for the base unit that divides most values, then check `lsf-better-layout`'s grouping rule. Is the gap between groups at least 2× the gap within one?
...
`variable: true` means one file covers a weight range. A class on `<html>` beside a `prefers-color-scheme` query means a toggle that can override the system, the mechanism `lsf-better-colors` describes.
