...
生成器把更多注意力放在前 30-40% 的 token 上。开头放主体与动作。风格修饰放末尾。运镜、灯光、环境放中间。
...
身份块必须包含：脸型、眼睛颜色、肤色。发色、长度、发型。胡须。确切的服装单品。标志性配饰。
...
大多数模型工作在 5-10 秒片段。更长的叙事活在剪辑器拼接的多片段里。别把 30 秒的故事塞进 5 秒的提示词。
