---
name: lsf-better-interface
title: LSF Better 界面
description: 跨领域界面总审查。把页面、流程或功能路由给所有 `lsf-better-*` 领域技能，并汇总成一个按优先级排序的结论。 用于用户要求整体审查而非单一领域，或
  `lsf-interface-review` 上交改动时。 触发词：lsf-better-interface、整体界面审查、审查整个页面、全面检查界面。
---
...
This skill runs a cross-discipline review. It routes the interface to each `lsf-better-*` skill, collects their evidence and consolidates one ranked verdict.

Orchestration is all it owns. Accessibility rules belong to `lsf-better-accessibility`, structure to `lsf-better-layout`, copy to `lsf-better-writing`, type to `lsf-better-typography`, color to `lsf-better-colors`, visual polish and motion to `lsf-better-ui`. Never duplicate or override their rules here.

Change-scoped review of uncommitted work, branches and pull requests belongs to `lsf-interface-review`, which resolves the scope and classifies findings before handing the review back.
...
### 2. Send a change to `lsf-interface-review`

A request naming a branch, pull request, commit range, or uncommitted changes is a change review, not a screen review. Say so and ask the user to run `lsf-interface-review`, which is user-invoked and cannot be started from here.

Never resolve a change scope here. Reading a diff, classifying findings and expanding changed files to affected surfaces belong to `lsf-interface-review`. Guess at them and the report has a scope nobody can check.

When `lsf-interface-review` hands a review back, it supplies the change scope, a status per finding and the change-scoped report format. Severity, ranking, the cap and the verdict stay here, and all three cover `Introduced` and `Regression` only.
...
1. `lsf-better-accessibility`
2. `lsf-better-layout`
3. `lsf-better-writing`
4. `lsf-better-typography`
5. `lsf-better-colors`
6. `lsf-better-ui`
