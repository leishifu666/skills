---
name: lsf-better-typography
title: 技能：LSF Better Typography
description: Web 排版。用于选择或搭配字体、建立字号阶梯，或处理组件中文本的样式与截断。触发词：排版、字体加载、WOFF2、可变字体、OpenType
  特性、字号阶梯、标题层级、行高、字间距、行长、text-wrap、截断、等宽数字、下划线、文本选择、iOS 输入框缩放、字体平滑、智能标点、text-box、首字下沉。
---
...
The words themselves belong to `lsf-better-writing`. Semantic heading structure belongs to `lsf-better-accessibility`. Spatial RTL layout and logical properties belong to `lsf-better-layout`. Contrast measurement belongs to `lsf-better-colors`. This skill owns how text renders, wraps and behaves in mixed-direction content.
...
When text looks low-contrast, use `lsf-better-colors` to measure the rendered pair and `lsf-better-accessibility` to classify the requirement. Leave the colors alone unless asked.
...
Set `lang` so browsers and assistive technology pick the right pronunciation, quotes and hyphenation. Set `dir` at the document or at the content boundary where direction changes. Preserve digit order, and use `<bdi>` to isolate a mixed-direction value. Spatial mirroring and logical CSS properties belong to `lsf-better-layout`.
