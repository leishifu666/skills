---
name: lsf-wewrite-style
description: 'LSF｜WeWrite 风格设置模块：首次使用引导（onboard）+ 重设公众号写作风格配置（style.yaml）。
...
metadata:
  ...
  github_path: skills/wewrite-style
  upstream_name: wewrite-style
...
> **LSF 汉化版**：上游模块名 `wewrite-style`，本机调用名 `lsf-wewrite-style`。
