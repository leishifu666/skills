---
name: lsf-interface-review
title: LSF 界面 审查
description: 审查一次改动而非一个静态页面：未提交工作、当前分支或拉取请求。只覆盖界面质量，不覆盖正确性、测试或安全。
...
Scope is all it owns. Domain rules belong to the `lsf-better-*` skills. Severity, consolidation, coverage, the cap and the verdict belong to `lsf-better-interface`, which this skill hands the review to.
...
The whole invocation is the target, so `/lsf-interface-review pr 482` reviews pull request 482. [Scope resolution](scope-resolution.md) holds the accepted targets and how each resolves.
...
- **A whole-repository interface audit**, which is not a change review. Hand it to `lsf-better-interface` as a repository-scope review, without this skill's scope block, statuses, or pre-existing section. With no change, every finding is pre-existing and the classification says nothing.
...
Hand every finding up with its status attached and let `lsf-better-interface` apply its cap and verdict rules.
...
### 7. Hand the review to `lsf-better-interface`

Hand `lsf-better-interface` the scope block, the affected surfaces and a status on every finding. It routes to the domain skills, applies severity, consolidates, enforces the cap and issues the verdict.

If `lsf-better-interface` is unavailable, report the resolved scope and the file inventory, name it as the missing skill and stop. Do not invent a severity scale, a cap, or a verdict.
...
| The severity scale or the finding cap restated here | Defer to `lsf-better-interface` |
...
End with `Block` when any `HIGH` remains and `Approve` otherwise, leaving the remaining findings in the table as work to do. When `lsf-better-interface` is available, the severity scale and the cap come from it.
