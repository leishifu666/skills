...
| --- | --- | --- |
| `aria-label`, `aria-labelledby`, `aria-describedby`, `aria-live`, `role=` | `lsf-better-accessibility` | The control or region lost its accessible name, description, or announcement |
| `alt=`, `<label`, `for=`, `scope=` | `lsf-better-accessibility` | Image, field, or table cell lost its programmatic association |
| `<button>`, `<a>`, `<nav>`, `<main>`, `<ul>` replaced by `div` or `span` | `lsf-better-accessibility` | Keyboard and assistive-technology behavior was traded for styling |
| `:focus-visible`, `:focus`, `outline`, `tabindex` | `lsf-better-accessibility` | Keyboard users lost the focus indicator or the element left the tab order |
| `prefers-reduced-motion`, `prefers-contrast` | `lsf-better-accessibility` | Motion or contrast now ignores the user's system preference |
| Logical properties swapped for `left` / `right` | `lsf-better-layout` | Direction-aware layout was dropped |
| `lang=`, `dir=` | `lsf-better-typography` | Language metadata or text direction was dropped |
| `text-wrap`, `line-clamp`, `overflow-wrap`, `tabular-nums`, `font-feature-settings` | `lsf-better-typography` | Text rendering, wrapping, or numeral alignment silently changed |
| A color token swapped for a literal, or a token swapped for a lighter one | `lsf-better-colors` | The rendered contrast pair may now fail; measure it |
| A user-facing string deleted or shortened | `lsf-better-writing` | A label, error, or empty state lost the information it carried |
