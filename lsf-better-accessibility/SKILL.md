---
name: lsf-better-accessibility
title: 技能：LSF Better Accessibility
description: 产品界面的无障碍工程。用于构建或审查 UI 组件、自定义控件，或处理键盘与屏幕阅读器问题。触发词：accessibility、a11y、WCAG、ARIA、焦点环、焦点陷阱、键盘导航、tabindex、屏幕阅读器、sr-only、替代文本、点击区域、触屏
  hover、prefers-reduced-motion、自动播放、跳转链接、语义化 HTML、表单错误、禁用按钮、无法用键盘操作。
---
...
Contrast measurement and color fixes belong to `lsf-better-colors`. Text sizing and iOS input zoom belong to `lsf-better-typography`. Spatial RTL layout belongs to `lsf-better-layout`.
...
Status needs a redundant cue: an icon, text, or an underline alongside the color. Work out which WCAG contrast requirement applies, then use `lsf-better-colors` to measure the rendered pair. When it fails, report the pair and the requirement it misses, and leave the colors alone unless asked.
